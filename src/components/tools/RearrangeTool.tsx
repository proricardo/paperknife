...
type RearrangePdfData = { file: File, pageCount: number, isLocked: boolean, pdfDoc?: any, password?: string }
...
const LazyThumbnail = ({ pdfDoc, pageNum }: { pdfDoc: any, pageNum: number }) => {
  ...
  if (src) return <img src={src} className="w-full h-full object-contain bg-white" alt={`P${pageNum}`} />
...
function SortablePage({ id, pageNum, pdfDoc }: { id: string, pageNum: number, pdfDoc: any }) {
  ...
  return (
    <div ref={setNodeRef} style={style} {...attributes} {...listeners} className={`relative group cursor-grab active:cursor-grabbing aspect-[3/4] rounded-xl overflow-hidden border-2 transition-all bg-gray-50 dark:bg-black ${isDragging ? 'border-rose-500 shadow-xl scale-105 opacity-90' : 'border-transparent hover:border-rose-500'}`}>
      <div className="w-full h-full p-2"><LazyThumbnail pdfDoc={pdfDoc} pageNum={pageNum} /></div>
      <div className="absolute top-2 right-2 bg-black/50 backdrop-blur-md rounded-lg p-1 text-white opacity-0 group-hover:opacity-100"><Move size={14} /></div>
...
export default function RearrangeTool() {
  ...
  const ActionButton = () => (
    <button onClick={savePDF} disabled={isProcessing} className={`w-full bg-rose-500 hover:bg-rose-600 text-white font-black uppercase tracking-widest transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-3 shadow-xl shadow-rose-500/20 ${isNative ? 'py-4 rounded-2xl text-sm' : 'p-6 rounded-3xl text-xl'}`}>
      {isProcessing ? <><Loader2 className="animate-spin" /> Working...</> : <>Save New Order <ArrowRight size={18} /></>}
    </button>
  )

  return (
    <NativeToolLayout title="Rearrange PDF" description="Drag and drop to reorder pages visually." icon={Grid} actions={pdfData && !pdfData.isLocked && !downloadUrl && <ActionButton />}>
      ...
      {!pdfData ? (
        <div onClick={() => !isProcessing && fileInputRef.current?.click()} className="border-4 border-dashed border-gray-100 dark:border-zinc-900 rounded-[2.5rem] p-12 text-center hover:bg-rose-50 dark:hover:bg-rose-900/10 transition-all cursor-pointer group">
          ...
          <p className="text-sm text-gray-400">Tap to start reordering</p>
        </div>
      ) : pdfData.isLocked ? (
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white dark:bg-zinc-900 p-6 rounded-[2rem] border border-gray-100 dark:border-white/5 shadow-sm">
              <div className="flex justify-between items-center mb-6">
                <h4 className="font-black uppercase tracking-widest text-[10px] text-gray-400">Drag to reorder</h4>
                <button onClick={() => setPageOrder(Array.from({ length: pdfData.pageCount }, (_, i) => (i + 1).toString()))} className="text-[10px] font-black uppercase text-gray-400 flex items-center gap-1"><RefreshCcw size={12}/> Reset</button>
              </div>
            ...
            <div className="bg-white dark:bg-zinc-900 p-8 rounded-[2rem] border border-gray-100 dark:border-white/5 shadow-sm sticky top-24">
              <div className="space-y-6">
                ...
                {!downloadUrl ? (!isNative && <ActionButton />) : (
                  <SuccessState message="Rearranged Successfully!" downloadUrl={downloadUrl} fileName={`${customFileName}.pdf`} onStartOver={() => setDownloadUrl(null)} />
                )}
